...
print '\nsys.args   :', sys.argv[1:]
inbam, inntrv, outfile, refGenome, tmpdir, gatk, outdir = sys.argv[1:]
cmd = 'java -Xms750m -Xmx5g -XX:+UseSerialGC -Djava.io.tmpdir=%(tmpdir)s -jar %(gatk)s -T IndelRealigner -I %(inbam)s -targetIntervals %(inntrv)s -o %(outfile)s -R %(refGenome)s %(dcov)s %(read_filter)s %(consensusDeterminationModel)s %(compress)s'
cmd = cmd % locals()
print cmd
