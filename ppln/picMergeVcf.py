...
N = 6
print '\nsys.args   :', sys.argv[1:]
refGenome, tmpdir, picarddir, outdir, outfile = sys.argv[1:N]
I = ' I='
infiles = ''
for f in sys.argv[N:]:
    infiles += I + f
...
cmd = 'java -Xms750m -Xmx5000m -XX:+UseSerialGC -jar '+picard+' %(infiles)s O=%(outfile)s TMP_DIR=%(tmpdir)s VALIDATION_STRINGENCY=%(VALIDATION_STRINGENCY)s CREATE_INDEX=%(CREATE_INDEX)s'
cmd = cmd % locals()
print cmd
