...
make -j $P -f ${srcdir}/callGATK_HC.mk INCLMK=$inclmk FAMCODE=$famcode INDIR=$workdir OUTDIR=$workdir LOGDIR=$outdir SUFFIX=-23 
ret=$?
echo $ret
if [ $ret -ne 0 ]; then
...
make -j $P -f ${srcdir}/callFreebayes.mk INCLMK=$inclmk FAMCODE=$famcode INDIR=$workdir OUTDIR=$workdir LOGDIR=$outdir SUFFIX=-23 
ret=$?
echo $ret
if [ $ret -ne 0 ]; then
...
make -j $P -f ${srcdir}/callPlatypus.mk INCLMK=$inclmk FAMCODE=$famcode INDIR=$workdir OUTDIR=$workdir LOGDIR=$outdir SUFFIX=-23
ret=$?
echo $ret
if [ $ret -ne 0 ]; then
